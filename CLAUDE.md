...
- `types.ts` / `protocol.ts`: `GameState`, `RoundState`, and the full `ClientMessage`/`ServerMessage` discriminated unions.
...
`players` (soft-delete via `archived_at`, never hard-deleted — historical games must still resolve archived players by id), `games`, `game_pool` (the 10-player draw + its randomized order for one game), `game_players` (final result, written by the DO when a game completes). `games` has unused `published_at`/`public_slug` columns reserved for a not-yet-built "publish results publicly for voting" feature — don't repurpose them.

### Frontend structure
`frontend/src/router.tsx` wires four routes (`/roster`, `/games/new`, `/game/:gameId/join`, `/game/:gameId`) to pages in `frontend/src/pages/`. Captain identity/session is `frontend/src/lib/session.ts` (localStorage, keyed by `gameId`, storing `{token, role}`) — there are no accounts; `POST /api/games` mints both captain tokens upfront, and the join URL embeds captain B's token as `?t=`. `frontend/src/hooks/useGameSocket.ts` is the sole place that opens/reconnects the game WebSocket and owns the reconnect policy (one immediate retry on unexpected close, then gives up — documented in that file). No CSS/styling has been added anywhere yet; components are unstyled semantic HTML by design choice so far, not oversight.
